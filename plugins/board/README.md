...
le catalogue est à la racine de ce repo, le plugin dans `plugins/board/`.

```
claude-code-plugins/               (repo = marketplace)
├── .claude-plugin/
│   └── marketplace.json           # catalogue : liste le plugin "board" → ./plugins/board
└── plugins/board/                 (racine du plugin)
    ├── .claude-plugin/
    │   └── plugin.json            # manifeste du plugin
    ├── .mcp.json                  # serveur MCP board (juste l'URL — auth OAuth au navigateur)
    ├── skills/
    │   ├── board-workflow/        # entrée : charge get_workflow, oriente la session
    │   ├── board-task/            # cycle de vie d'une tâche (request → tasks, présence)
    │   ├── board-watch/           # boucle de veille : commentaires + auto-merge
    │   └── board-report/          # reporting selon reportPrompt + cadence
    └── README.md
```
...
> `--path`. Le plugin, lui, peut vivre dans un sous-dossier (`source: ./plugins/board`).
...
Board** (login email/mdp ou « Continuer avec mon compte Board » → Google/GitHub). Aucune
`BOARD_API_KEY` à exporter côté client (cf. [ADR 0014](https://github.com/badjilounes/board/blob/main/docs/adr/0014-oauth-mcp-http-connecteur-claude-ai.md) / [0015](https://github.com/badjilounes/board/blob/main/docs/adr/0015-login-board-connecteur-mcp-pont-consentement-web.md)).
...
> **Connect** (onglet Overview) de l'app Board.
...
```bash
/mcp     # doit lister "board" comme connecté
```
...
> des « Connectors ». Ajoute Board via Settings → Connectors → Add custom connector →
> `https://mcp-production-b5f9.up.railway.app/mcp` ; le connecteur lance **le même OAuth**
