...
name: board-watch
description: >-
  Run the Board watch loop — re-read task and request comments and act on them, and apply
...
comment asks for something, do it, then reflect the result on Board — a status change, a
reply comment, or a work note.

## Auto-merge — apply `automation.autoMergeMode`

Board never reads your CI and never merges — **you** do. In every non-`none` case you merge
the PR yourself (e.g. `gh pr merge`), attach a `ci` artifact with source `observed` on the
task's execution, then move the task to the workflow's terminal status; Board records the
merge.
