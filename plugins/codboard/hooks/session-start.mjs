...
function trackedContext(cfg) {
  ...
  return [
    ...
    '     ciCheckName } and `technology`. Apply the policy of the repository the task belongs',
    '     to: two repositories of one project may answer differently. `technology` decides the',
    '     NATURE of the proof its tasks must attach, and the tool that produces it.',
    ...
    '   (`set_task_pull_request`), test plan (`add_test_step`), capture — of the nature the',
    '   technology demands (`create_media_upload` for what is looked at, a `text` media',
    '   carrying the content for what is read) —, done (`change_task_status` ->',
    '   `complete_execution`), report (`upsert_report`).',
    ...
    '',
    'Enforcement, in BOTH directions — merging what you must not, and not merging what you must.',
    'The turn is BLOCKED from ending while this session has changed code with no run open, while',
    'a created or pushed branch or an opened PR is unmirrored, while a PR you opened under a',
    'non-`none` `autoMergeMode` is left neither merged nor accounted for (log the failing barrier',
    'or park the task in a blocked status), or while the daily report is stale versus',
    '`reportingCadence`; and a merge that violates the repository `autoMergeMode` is blocked',
    'whether it goes through `gh pr merge` or the GitHub MCP server.',
    ...
    'Its `proofExpectation` also says WHICH proof: a Playwright capture for a frontend, the cURL',
    'response for a backend, a Maestro capture for mobile, the response of an LLM call for an MCP',
    'server, the rendered content for a documentation ticket. A repository typed `monorepo` has no',
    'technology of its own: declare the ones the change touches (`set_task_technologies`) — nothing',
    'else can, CodBoard never sees your files — or the move is refused for the missing declaration.',
  ]
