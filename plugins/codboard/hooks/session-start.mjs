...
function trackedContext(cfg) {
  ...
  return [
    ...
    '1. Call `get_workflow` NOW and follow it. Its statuses, transitions,',
    '   playbook, `automation` (incl. `autoMergeMode`), `testing` and',
    '   `reportPrompt` are the SOURCE OF TRUTH — read them at runtime, never',
    '   copy them into CLAUDE.md or any repo file (they change per project).',
    ...
    '   (`set_task_pull_request`), test plan (`add_test_step`), proof',
    ...
    '',
    'Enforcement: local hooks record git/PR milestones and will block the turn',
    'from ending while a milestone is still unsynced, and will block a merge that',
    'violates `autoMergeMode`. Keep the board in sync to avoid being blocked.',
  ]
